...
AUGMENT_ID=0
for augment in "${AUGMENT_1_ARRAY[@]}" "${AUGMENT_2_ARRAY[@]}" "${AUGMENT_3_ARRAY[@]}"; do
    echo "$augment"
    ...
    AUGMENT_STATS="{" 
    if grep -e "gain[s]* [0-9]* [Hh]ealth" <(echo "$AUGMENT_DESC"); then
        AUGMENT_STATS+='"health": '"$(echo "$AUGMENT_DESC" | sed -n 's|.*gain[s]* \([0-9]*\) [Hh]ealth.*|\1|p')"","
    else 
    ...
    if grep -e "Attack Speed" <(echo "$AUGMENT_DESC"); then
        AUGMENT_STATS+='"speed": '"$(echo "$AUGMENT_DESC" | sed 's|.* \([0-9%][0-9%.-]*\) .*Attack Speed.*|\1 |')"","
    else 
